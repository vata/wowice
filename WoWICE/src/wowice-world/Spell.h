...
WoWICE_INLINE bool CanAgroHash(uint32 spellhashname)
{
...
/************************************************************************/
WoWICE_INLINE bool IsDamagingSpell(SpellEntry *sp)
{
    switch (sp->Effect[0])
    {
...
WoWICE_INLINE uint32 IsHealingSpell(SpellEntry *sp)
{
    switch( sp->Effect[0] )
    {
        case SPELL_EFFECT_HEALTH_LEECH:
...
WoWICE_INLINE bool IsInrange(LocationVector & location, Object * o, float square_r)
{
...
WoWICE_INLINE bool IsInrange(float x1,float y1, float z1,float x2,float y2, float z2,float square_r)
{
    float t;
    float r;
    t=x1-x2;
...
WoWICE_INLINE bool IsInrange(Object * o1,Object * o2,float square_r)
{
    return IsInrange(o1->GetPositionX(),o1->GetPositionY(),o1->GetPositionZ(),
        o2->GetPositionX(),o2->GetPositionY(),o2->GetPositionZ(),square_r);
}

WoWICE_INLINE bool TargetTypeCheck(Object *obj,uint32 ReqCreatureTypeMask)
{
...
WoWICE_INLINE bool IsTargetingStealthed(SpellEntry *sp)
{
	if(
		HasTargetType(sp,EFF_TARGET_INVISIBLE_OR_HIDDEN_ENEMIES_AT_LOCATION_RADIUS) ||
		HasTargetType(sp,EFF_TARGET_ALL_ENEMIES_AROUND_CASTER) ||
...
public:
	...
	WoWICE_INLINE bool hasAttributeEx(uint32 attribute)
	{
		return ((GetProto()->AttributesEx & attribute) > 0);
	}
	WoWICE_INLINE bool hasAttributeExB(uint32 attribute)
	{
		return ((GetProto()->AttributesExB & attribute) > 0);
	}
	WoWICE_INLINE bool hasAttributeExC(uint32 attribute)
	{
    ...
    WoWICE_INLINE uint32 getState() { return m_spellState; }
    WoWICE_INLINE void SetUnitTarget(Unit *punit){unitTarget=punit;}
...
//	void SpellEffectActivateRune(uint32 i);
    ...
    WoWICE_INLINE Item* GetItemTarget() { return itemTarget; }
    WoWICE_INLINE Unit* GetUnitTarget() { return unitTarget; }
    WoWICE_INLINE Player* GetPlayerTarget() { return playerTarget; }
    WoWICE_INLINE GameObject* GetGameObjectTarget() { return gameObjTarget; }
	...
	WoWICE_INLINE SpellEntry* GetProto() { return (m_spellInfo_override == NULL) ?  m_spellInfo : m_spellInfo_override; }
    ...
    WoWICE_INLINE float GetRadius(uint32 i)
    {
    ...
    WoWICE_INLINE static uint32 GetBaseThreat(uint32 dmg)
    {
...
/*        switch(GetProto()->NameHash)
	...
	WoWICE_INLINE static uint32 GetMechanic(SpellEntry * sp)
	{
		if(sp->MechanicsType)
	...
	WoWICE_INLINE void safe_cancel()
	{
		m_cancelled = true;
	}
    ...
    /// Spell state's
    /// Spell failed
    WoWICE_INLINE bool GetSpellFailed(){return m_Spell_Failed;}
    WoWICE_INLINE void SetSpellFailed(bool failed = true){m_Spell_Failed = failed;}

    WoWICE_INLINE bool IsReflected() {return m_IsReflected;}
    WoWICE_INLINE void SetReflected(bool reflected = true) {m_IsReflected = reflected;}

    /// Spell possibility's
    WoWICE_INLINE bool GetCanReflect() {return m_CanRelect;}
    WoWICE_INLINE void SetCanReflect(bool reflect = true) {m_CanRelect = reflect;}
...
protected:
	...
	WoWICE_INLINE bool HasTarget(const uint64& guid, TargetsList* tmpMap)
	{

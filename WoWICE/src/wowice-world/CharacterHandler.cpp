...
void WorldSession::CharacterEnumProc(QueryResult * result)
{
	...
	if(result)
	{
		...
		do
		{
			...
			{
				res = CharacterDatabase.Query("SELECT entry, level FROM playerpets WHERE ownerguid = %u AND MOD( active, 10 ) = 1 AND alive = TRUE;", Wowice::Util::GUID_LOPART( guid ) );
			...
			res = CharacterDatabase.Query("SELECT slot, entry, enchantments FROM playeritems WHERE ownerguid=%u AND containerslot = '-1' AND slot BETWEEN '0' AND '22'", Wowice::Util::GUID_LOPART( guid ));
...
void WorldSession::FullLogin(Player * plr)
{
	...
	{
		Transporter * pTrans = objmgr.GetTransporter( Wowice::Util::GUID_LOPART(plr->m_TransporterGUID));
		if(pTrans)
		{
			if(plr->IsDead())
			{

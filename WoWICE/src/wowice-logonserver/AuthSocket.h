...
public:
	...
	WoWICE_INLINE sAuthLogonChallenge_C * GetChallenge() { return &m_challenge; }
	WoWICE_INLINE void SendPacket(const uint8* data, const uint16 len) { Send(data, len); }
	void OnDisconnect();
	WoWICE_INLINE time_t GetLastRecv() { return last_recv; }
	bool removedFromSet;
	WoWICE_INLINE uint32 GetAccountID() { return m_account ? m_account->AccountId : 0; }

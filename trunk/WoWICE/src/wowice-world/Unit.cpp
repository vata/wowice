...
void Unit::SendAuraUpdate( uint32 AuraSlot, bool remove ){
	...
	Wowice::Util::WOWICE_ASSERT( aur != NULL );
...
void CombatStatusHandler::AddAttackTarget(const uint64& guid)
{
	...
	//we MUST be in world
	Wowice::Util::WOWICE_ASSERT( m_Unit->IsInWorld() );
...
void Unit::RemoveGuardianRef( Creature* g )
{
    ...
    Wowice::Util::WOWICE_ASSERT(    g != NULL );
...
void Unit::AddGarbagePet( Pet *pet ){
	Wowice::Util::WOWICE_ASSERT( pet->GetPetOwner()->GetGUID() == GetGUID() && !pet->IsInWorld() );
